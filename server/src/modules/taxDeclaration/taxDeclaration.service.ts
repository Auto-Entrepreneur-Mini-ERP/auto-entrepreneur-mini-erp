...
const getAllTaxDeclarations = async (autoEntrepreneurId: string) => {
...
const createTaxDeclaration = async (data : TaxDeclaration) => {
...
const updateTaxDeclaration = async (id: string, data:PatchTaxDeclarationDTO) => {
  ...
  if (!existing) {
   ...
   if ("month" in data) updateData.month = data.month ?? null;
   if ("totalRevenue" in data) updateData.totalRevenue = data.totalRevenue ?? 0;
   if ("taxRate" in data) updateData.taxRate = data.taxRate ?? 0;
   if ("taxAmount" in data) updateData.taxAmount = data.taxAmount ?? 0;
   if ("status" in data) updateData.status = data.status ?? "DRAFT";
   if ("dueDate" in data) updateData.dueDate = data.dueDate ?? null;
   if ("paymentDate" in data) updateData.paymentDate = data.paymentDate ?? null;
...
const getTaxDeclaration = async (id: string) => {
