...
export const registerNotificationCrons = () => {
  ...
  cron.schedule('0 8 * * *', async () => {
    ...
    for (const decl of declarations) {
      await notificationService.createEcheanceNotification(
        decl.AutoEntrepreneurId,
        'tax',
        decl.id,
        `Déclaration ${decl.period} ${decl.year}`,
        TAX_WARNING_DAYS,
      );
    }
  });
  ...
  cron.schedule('0 8 * * *', async () => {
    ...
    for (const contrib of contributions) {
      await notificationService.createEcheanceNotification(
        contrib.AutoEntrepreneurId,
        'contribution',
        contrib.id,
        `Cotisation CNSS ${contrib.period} ${contrib.year}`,
        CNSS_WARNING_DAYS,
      );
    }
  });
  ...
  cron.schedule('0 9 * * 1', async () => {
    ...
    for (const invoice of overdueInvoices) {
      await notificationService.createRetardNotification(
        invoice.AutoEntrepreneurId,
        'invoice',
        invoice.id,
