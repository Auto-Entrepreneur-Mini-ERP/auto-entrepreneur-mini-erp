...
export class ExpenseService {
    ...
    // ── GET ALL ────────────────────────────────────────────────────────────────
    async getExpenses(autoEntrepreneurId, filters, page = 1, limit = 20) {
    ...
    // ── GET BY ID ──────────────────────────────────────────────────────────────
    async getExpenseById(id, autoEntrepreneurId) {
    ...
    // ── UPDATE ─────────────────────────────────────────────────────────────────
    async updateExpense(id, autoEntrepreneurId, data) {
        const existing = await prisma.expense.findFirst({
            where: { id, AutoEntrepreneurId: autoEntrepreneurId },
        });
        if (!existing)
            return null;
        if (data.receiptUrl && existing.receiptUrl)
            this._deleteFile(existing.receiptUrl);
        const updateData = {};
        ...
        if (data.isDeductible !== undefined)
            updateData.isDeductible = data.isDeductible;
    ...
    // ── DELETE ─────────────────────────────────────────────────────────────────
    async deleteExpense(id, autoEntrepreneurId) {
        const existing = await prisma.expense.findFirst({
            where: { id, AutoEntrepreneurId: autoEntrepreneurId },
        });
        if (!existing)
    ...
    // ── STATS ──────────────────────────────────────────────────────────────────
    async getStats(autoEntrepreneurId, startDate, endDate) {
    ...
    // ── EXPORT EXCEL ───────────────────────────────────────────────────────────
    async exportToExcel(autoEntrepreneurId, filters) {
        const where = this._buildWhere(autoEntrepreneurId, filters);
    ...
    // ── PRIVATE HELPERS ────────────────────────────────────────────────────────
    _buildWhere(autoEntrepreneurId, filters) {
        const where = { AutoEntrepreneurId: autoEntrepreneurId };
        if (filters?.category)
